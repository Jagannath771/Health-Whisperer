...
def _ctx_flags(uid: str) -> dict:
    ...
    try:
        ...
        def any_of(words): return any(w in blob for w in words)

        return {
            "stress": any_of(["stress", "stressed", "anxiety", "anxious", "overwhelm"]),
            "injury": any_of(["injury", "sprain", "strain", "sore", "pain", "ache"]),
            "fasting": any_of(["fasting", "intermittent fast"]),
            "dehydrated": any_of(["dehydrated", "dry mouth", "thirsty", "headache"]),
            "late_night_eating": any_of(["late dinner", "midnight snack", "11pm", "12am"]),
        }
    except Exception:
        return {}
    ...
    if flags.get("stress") and 8 <= now_l.hour <= 22:
        if not any(n.get("type") in ("breathing","stress_relief") for n in nudges):
            nudges.insert(0, {
                ...
                "icon": "🌬️",
                "title": "60-sec reset",
                "msg": "Feeling stressed? Box breathing: 4-in / 4-hold / 4-out / 4-hold × 8.",
